package com.vaddya.fpscala.principles.tweetset
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val children = left.filter(p).union(right.filter(p))
  ...
  def mostRetweeted: Tweet = {
    ...
    if (left.nonEmpty && leftTweet.retweets > elem.retweets)
      if (right.nonEmpty && rightTweet.retweets > leftTweet.retweets) rightTweet
      else leftTweet
    else if (right.nonEmpty && rightTweet.retweets > elem.retweets)
      rightTweet
    else
      elem
  }
